...
public class GeoHR3 {
    ...
    static float DEFAULT_THRESHOLD = 1f;
    static int DEFAULT_GRANULARITY = 4;
    protected int granularity;
    protected float angularThreshold;
    protected float distanceThreshold;
    public static Logger logger = Logger.getLogger(GeoHR3.class.getName());
    static float delta;
    int latMax, latMin, longMax, longMin;
    public boolean HR3;
    public Hausdorff hausdorff;
    public boolean useFastHausdorff = true;
    public boolean verbose = false;

    public GeoHR3(float distanceThreshold, int granularity, HausdorffFactory.Type hd) {
//        this.angularThreshold = (float)Math.ceil(1000*distanceThreshold*180/(Math.PI*OrthodromicDistance.R))/1000f;
...
//        System.out.println("Max(long) " + longMax);
        ...
        //initialize the Hausdorff implementation to use
        hausdorff = HausdorffFactory.getHausdorff(hd);
//        System.out.println("Using " + hausdorff.getName() + " approach.");
    ...
    public Set<List<Integer>> getSquaresToCompare(int latIndex, int longIndex, GeoIndex index) {
        ...
        for (int deltaLat = (-1) * granularity; deltaLat <= granularity; deltaLat++) {
            ...
            // else take the one below. Euqivalent to taking the latitude circle with the largest radius
            else {
                if (realLat < 0) {
                ...
                //if crossing ocurred we need to alter the longIndex
                if (polarCross < 0) {
                    lon = -1 - longIndex;
                } else {
                    lon = longIndex;
...
//                    }
        ...
        if (HR3) {
            ...
            for (List<Integer> candidate : toCompare) {
                ...
                if (latIndex == candidate.get(0) && longIndex == candidate.get(1)) {
                    ...
                    float d = OrthodromicDistance.getDistanceInDegrees(lat1, long1, lat2, long2);
    ...
    /**
     * Runs GeoHR3 for source and target dataset. Uses the set Hausdorff
...
//        System.out.println("Geo-Indexing took: " + (end - begin) + " ms");
...
//        System.out.println("Angular Threshold = " + angularThreshold);
        Mapping m = new Mapping.NoOpMapping();

        float d;
        if (hausdorff instanceof CentroidIndexedHausdorff) {
            ((CentroidIndexedHausdorff) hausdorff).computeIndexes(sourceData, targetData);
        } else if (hausdorff instanceof IndexedHausdorff) {
            PolygonIndex targetIndex = new PolygonIndex();
            targetIndex.index(targetData);
            ((IndexedHausdorff) hausdorff).targetIndex = targetIndex;
        }
...
//                                if (source.getIndexes(a).containsAll(target.getIndexes(b))) {
                                    d = hausdorff.computeDistance(a, b, distanceThreshold);
                                    if (d <= distanceThreshold) {
                                        m.add(a.uri, b.uri, d);
                                    }
//                                }
